function pixel_chart(){
	...
	// create all sliders
	const duration_histogram_slider = new Duration_Histogram_Slider('pixel_filter', ...d3.extent(data.map(d => d.duration)), data.map( d => d.duration))
	duration_histogram_slider.create(update)

	const time_histogram_slider = new Time_Histogram_Slider('pixel_filter', d3.min(data.map( d => d.date_added)), data.map(d => d.date_added))
	time_histogram_slider.create(update)

	const averageRating_histogram_slider = new AverageRating_Histogram_Slider('pixel_filter', data.map(d => d.averageRating))
	averageRating_histogram_slider.create(update)

	const numVotes_histogram_slider = new numVotes_Histogram_Slider('pixel_filter', d3.max(data.map(d => d.numVotes)), data.map(d => d.numVotes))
	...
	function get_all_condition_and_prepare_data(){
		...
		filters.selectAll('#pixel_filter div.selected_country div').each(function(){ selected_countrys.push(d3.select(this).attr('data-country')) })
		//filters.select('selected_countrys')

		// get selected genres
		let selected_genres = []
		filters.selectAll('#pixel_filter div.selected_genre div').each(function(){ selected_genres.push(d3.select(this).attr('data-genre')) })
		...
		selected_countrys.map( c => map_data.set(c, filtered_data.filter( function(d){
	...
	function draw_images(data, x_amount, countrys, genres){
		d3.selectAll('#my_dataviz g.view ').remove()
		xGroup.domain(d3.range(data.size))

		const image_group = svg.selectAll('g.view')
			.data(data.keys())
			.join('g')
